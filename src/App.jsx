...
function TaskForm({ task, columns, defaultColumn, defaultDate, onSave, onClose }) {
  const [form, setForm] = useState(() => {
    ...
    return {title:'',description:'',notes:'',link:'',start:date,due:date,columnId:defaultColumn||columns[0].id,effort:3};
  });
...
function TaskCard({ task, onEdit, onArchive, onDragStart }) {
  ...
  return <article className={`task-card ${task.status} ${overdue?'overdue':''}`} draggable onDragStart={e=>onDragStart(e, task.id)} onClick={()=>onEdit(task)}>
...
function Board({ columns, tasks, setTasks, setColumns, openTask, onViewTimeline }) {
  ...
  return <div className="board-wrap"><div className="board-header"><div><p className="eyebrow">RICARDO ALFARO</p><h1>Tablero de estado</h1><p className="subtitle">Organiza el trabajo de hoy y mantén el foco.</p></div><button className="primary add-top" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>
    ...
    <div className="board" role="list">{columns.map(col=>{const list=tasks.filter(t=>t.columnId===col.id&&t.status!=='deprecated');const isFixed=isFixedColumn(col);return <section className={`column ${isFixed?'fixed-column':'custom-column'}`} key={col.id} draggable={!isFixed} onDragStart={event=>{if(!isFixed)event.dataTransfer.setData('text/column',col.id)}} onDragOver={e=>e.preventDefault()} onDrop={event=>handleColumnDrop(event,col.id)}>
      <header className="column-header"><div className="column-title"><span style={{background:col.color}}></span><EditableColumnTitle value={col.title} onChange={title=>renameColumn(col.id,title)}/><b className={`status-count status-${columnTone(col)}`}>{list.length}</b></div>{!isFixed&&<div className="column-menu-wrap"><button className="icon-button" onClick={()=>setColumnMenu(current=>current===col.id?null:col.id)} aria-label={`Opciones de ${col.title}`} aria-expanded={columnMenu===col.id}><DotsThree width={22} height={22}/></button>{columnMenu===col.id&&<div className="column-menu"><button onClick={()=>deleteColumn(col.id)}><Trash width={16} height={16}/> Eliminar columna</button></div>}</div>}</header>
...
function Timeline({ tasks, columns, setTasks, openTask, holidays, setHolidays, initialAnchorDate }) {
  ...
  const visibleTasks=tasks.filter(task=>task.status!=='deprecated'&&(task.start||task.due)<=workEnd&&task.due>=range.start).sort((a,b)=>(a.start||a.due).localeCompare(b.start||b.due)||b.due.localeCompare(a.due));
...
export function App() {
  ...
  </aside><main>{cloudError&&<div className="cloud-alert" role="alert">{cloudError}</div>}{page==='board'?<Board columns={columns} tasks={tasks} setTasks={setTasks} setColumns={setColumns} openTask={openTask} onViewTimeline={date=>{setTimelineAnchorDate(date);setPage('timeline')}}/>:page==='timeline'?<Timeline tasks={tasks} columns={columns} setTasks={setTasks} openTask={openTask} holidays={holidays} setHolidays={setHolidays} initialAnchorDate={timelineAnchorDate}/>:page==='today'?<Today tasks={tasks} setTasks={setTasks} openTask={openTask} doneColumnId={doneColumnId}/>:page==='archive'?<ArchiveView tasks={tasks} columns={columns} setTasks={setTasks} openTask={openTask}/>:page==='settings'?<AccessSettings/>:<Reports tasks={tasks} openTask={openTask} canShare={Boolean(access?.canManageAccess)}/>}</main>

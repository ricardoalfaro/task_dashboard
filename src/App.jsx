...
function ProjectSwitcher() {
  return <label className="project-switcher"><span>Proyecto</span><select value="ricardo-alfaro" onChange={()=>{}} aria-label="Proyecto actual"><option value="ricardo-alfaro">Ricardo Alfaro</option></select></label>;
}
...
export function App() {
  ...
  if(access?.role==='viewer'||isLocalReportPreview)return <div className="viewer-shell"><header><div><p className="eyebrow">{isLocalReportPreview?'VISTA PREVIA LOCAL · SOLO LECTURA':'REPORTE COMPARTIDO · SOLO LECTURA'}</p><strong>{boardTitle}</strong></div><div className="viewer-header-actions"><button className="secondary viewer-theme-toggle" onClick={cycleTheme} title={`${themeLabel}. Cambiar apariencia`} aria-label={`${themeLabel}. Cambiar apariencia`}><ThemeIcon width={18} height={18}/><span>{themeLabel}</span></button>{!isLocalReportPreview&&<button className="secondary" onClick={()=>authRepository.signOut()}>Cerrar sesión</button>}</div></header>{cloudError&&<div className="cloud-alert" role="alert">{cloudError}</div>}<main><Reports tasks={tasks} openTask={task=>{setEditing(task);setModal(true)}}/></main>{modal&&editing&&<Modal title="Detalle de tarea" onClose={()=>setModal(false)}><ReadOnlyTask task={editing} columnTitle={columns.find(column=>column.id===editing.columnId)?.title}/></Modal>}</div>;

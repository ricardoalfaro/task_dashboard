...
function Board({ columns, tasks, setTasks, setColumns, openTask, onViewCompleted }) {
  ...
  return <div className="board-wrap"><div className="board-header"><div><ProjectSwitcher/><h1>Tablero</h1><p className="subtitle">Organiza el trabajo de hoy y mantén el foco.</p></div><button className="primary add-top" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>
...
function Today({ tasks, setTasks, openTask }) {
  const today = toISODate(new Date());
  const list = tasks.filter(task => task.status !== 'deprecated' && (task.due === today || (task.status==='active' && task.due<today))).sort((a,b)=>{const aOverdue=a.status==='active'&&a.due<today;const bOverdue=b.status==='active'&&b.due<today;return Number(bOverdue)-Number(aOverdue)||a.due.localeCompare(b.due)});
  ...
  return <div className="today-view">
    ...
    <section className="today-panel">
      <div className="today-list">{list.length ? list.map(task => <article className={`today-row home-task-row ${task.status==='active'&&task.due<today?'overdue':''}`} key={task.id} onClick={() => openTask(task)}>
        ...
        <span className={`today-status ${task.status}`}>{task.status === 'completed' ? 'DONE' : task.columnId === 'doing' ? 'DOING' : 'TO-DO'}</span>
...
export function App() {
  ...
  const todayTaskCount=useMemo(()=>{const today=toISODate(new Date());return tasks.filter(task=>task.status!=='deprecated'&&(task.due===today||(task.status==='active'&&task.due<today))).length},[tasks]);

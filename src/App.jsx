...
import {
  Archive, Book,
  Calendar as CalendarBlank, Reports as ChartBar, Check, CheckCircle, TaskList as ClipboardText,
...
function Board({ columns, tasks, setTasks, setColumns, openTask, onViewTimeline, onViewCompleted }) {
  ...
  const toggleChecklist = (task,itemId) => setTasks(items=>items.map(current=>{if(current.id!==task.id)return current;const checklist=normalizeChecklist(current.checklist,current.description,current.id).map(item=>item.id===itemId?{...item,checked:!item.checked}:item);const hasStarted=checklist.some(item=>item.checked);const singleItemChecklist=checklist.length===1;const columnId=!hasStarted?todoColumnId:singleItemChecklist&&checklistComplete(checklist)?doneColumnId:current.columnId===doneColumnId&&checklistComplete(checklist)?doneColumnId:doingColumnId;return {...current,checklist,description:checklistDescription(checklist),columnId,status:columnId===doneColumnId?'completed':'active',completedAt:columnId===doneColumnId?(current.completedAt||new Date().toISOString()):undefined,updatedAt:new Date().toISOString()}}));
  ...
  const overdueTasks=tasks.filter(task=>task.status==='active'&&task.due<toISODate(new Date())).sort((a,b)=>a.due.localeCompare(b.due));
  ...
  return <div className="board-wrap"><div className="board-header"><div><ProjectSwitcher/><h1>Tablero de estado</h1><p className="subtitle">Organiza el trabajo de hoy y mantén el foco.</p></div><button className="primary add-top" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>
    {overdueCount>0&&<div className="overdue-alert" role="status"><span>Tienes {overdueCount} {overdueCount===1?'tarea vencida':'tareas vencidas'}</span><button onClick={()=>onViewTimeline(overdueTasks[0].due)}>Ver</button></div>}
...
function Timeline({ tasks, columns, setTasks, openTask, holidays, setHolidays, initialAnchorDate }) {
  ...
  return <div className="timeline-view"><div className="timeline-header"><div><ProjectSwitcher/><div className="timeline-title"><h1>Planificación semanal</h1><span>{visibleTasks.length} {visibleTasks.length===1?'tarea':'tareas'}</span></div><p className="subtitle">Distribuye el trabajo y revisa la carga de cada día.</p></div><button className="primary" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>
...
function Reports({ tasks, openTask, canShare=false, hideShare=false }) {
  ...
  const data=[{key:'completed',label:'Terminadas',value:completed,color:themeColors.primary,icon:Check},{key:'pending',label:'Pendientes',value:pending,color:themeColors.interaction,icon:ClipboardText}];
  const detailTasks=openDetail==='completed'?completedTasks:pendingTasks;
  const detailTitle=openDetail==='completed'?'Tareas terminadas':'Tareas pendientes';
  ...
  return <div className="reports"><div className="reports-header"><div className="report-tabs" role="tablist" aria-label="Secciones del reporte"><button type="button" role="tab" aria-selected="true">Resumen General</button></div><div className="reports-controls">{!hideShare&&<button className="share-report" disabled={!canShare} onClick={shareReport} title={canShare?'Copiar URL privada del reporte':'Disponible al conectar el acceso externo'}><ShareAndroid width={18} height={18}/> {shareState||'Compartir'}</button>}<div className="period-controls"><select aria-label="Año del reporte" value={anchor.getFullYear()} onChange={event=>setPeriodYear(event.target.value)}>{years.map(year=><option key={year}>{year}</option>)}</select><select aria-label="Período del reporte" value={selectedPeriod} onChange={event=>choosePeriod(event.target.value)}><optgroup label="Meses">{months.map((month,index)=><option key={month} value={`month:${index+1}`}>{month}</option>)}</optgroup><optgroup label="Trimestres">{[1,2,3,4].map(value=><option key={value} value={`quarter:${value}`}>Q{value}</option>)}</optgroup><optgroup label="Semanas">{Array.from({length:53},(_,index)=><option key={index+1} value={`week:${index+1}`}>Semana {index+1}</option>)}</optgroup></select><button type="button" className="period-picker-trigger" onClick={()=>setDatePickerOpen(true)} aria-label="Elegir fecha"><CalendarBlank width={18} height={18}/></button></div></div></div>
    ...
    <div className="report-cards">{data.map(item=><article className={`report-card-action ${openDetail===item.key?'is-expanded':''}`} key={item.key} onClick={()=>setOpenDetail(current=>current===item.key?null:item.key)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();setOpenDetail(current=>current===item.key?null:item.key)}}} role="button" tabIndex={0} aria-expanded={openDetail===item.key}><div className="metric-icon" style={{color:item.color,background:`${item.color}14`}}><item.icon width={22} height={22}/></div><span>{item.label}</span><strong>{item.value}</strong><small>Ver tareas · {Math.round(item.value/total*100)}% del total del período</small></article>)}</div>
    {openDetail&&<section className={`completed-report report-detail is-${openDetail}`}><header><div><p className="eyebrow">DETALLE DEL PERÍODO</p><h2>{detailTitle}</h2></div><span>{detailTasks.length}</span></header><div>{detailTasks.length?detailTasks.map(task=>{const date=openDetail==='completed'?task.completedAt:task.due;const StatusIcon=openDetail==='completed'?Check:CalendarBlank;return <button key={task.id} onClick={()=>openTask(task)}><div><b>{task.title}</b><p>{task.description||'Sin descripción'}</p></div><span><StatusIcon width={15} height={15}/> {formatPeriodDate(date.slice(0,10))}</span></button>}):<div className="completed-empty">No hay tareas {openDetail==='completed'?'terminadas':'pendientes'} en este período.</div>}</div></section>}
...
function Today({ tasks, setTasks, openTask, doneColumnId='done' }) {
  ...
  return <div className="today-view">
    <div className="today-header">
      <div><ProjectSwitcher/><h1>Tareas para hoy <span className="page-title-count">{list.length}</span></h1><p className="subtitle">Todo lo que necesita tu atención hoy.</p></div>
...
function CompletedTasks({ tasks, setTasks, openTask }) {
  ...
  return <div className="today-view completed-tasks-view"><div className="today-header"><div><ProjectSwitcher/><h1>Tareas terminadas <span className="page-title-count">{list.length}</span></h1><p className="subtitle">Revisa todas las tareas que ya completaste.</p></div></div>
    <section className="today-panel"><div className="today-list">{list.length?visibleTasks.map(task=>{const completedDate=(task.completedAt||task.due).slice(0,10);return <article className="today-row completed-task-row" key={task.id} onClick={()=>openTask(task)}><span className="today-check checked" aria-hidden="true"><Check width={14} height={14} strokeWidth={2.2}/></span><div className="today-copy"><h3>{task.title}</h3><p>{task.description||'Sin detalles adicionales.'}</p></div><span className="today-status completed">Terminada</span><span className="today-date"><CalendarBlank width={15} height={15}/> {formatPeriodDate(completedDate)}</span><button className="icon-button archive-completed-task" onClick={event=>{event.stopPropagation();setArchivingTask(task)}} aria-label={`Archivar ${task.title}`} title="Archivar"><Trash width={19} height={19}/></button></article>}) : <div className="today-empty"><Check width={32} height={32}/><h3>Aún no hay tareas terminadas</h3><p>Las tareas que completes aparecerán aquí.</p></div>}</div>{list.length>perPage&&<footer className="completed-pagination"><span>Página {currentPage+1} de {totalPages}</span><div><button className="icon-button" disabled={currentPage===0} onClick={()=>setPage(current=>Math.max(0,current-1))} aria-label="Página anterior"><NavArrowLeft width={20} height={20}/></button><button className="icon-button" disabled={currentPage===totalPages-1} onClick={()=>setPage(current=>Math.min(totalPages-1,current+1))} aria-label="Página siguiente"><NavArrowRight width={20} height={20}/></button></div></footer>}</section>{archivingTask&&<Modal title="Archivar tarea" onClose={()=>setArchivingTask(null)}><p className="move-prompt-copy">¿Quieres archivar “{archivingTask.title}”? Podrás restaurarla desde Archivo.</p><footer className="modal-actions"><button type="button" className="secondary" onClick={()=>setArchivingTask(null)}>Cancelar</button><button className="primary" onClick={archiveTask}>Archivar</button></footer></Modal>}</div>;
}
...
export function App() {
  ...
  return <div className={`app-shell ${sidebarCollapsed?'sidebar-collapsed':''}`}><aside><div className="account"><label className="sidebar-search"><MagnifyingGlass width={18} height={18}/><input type="search" placeholder="Buscar" aria-label="Buscar"/></label><button className="account-sidebar-toggle" onClick={()=>setSidebarCollapsed(value=>!value)} aria-label={sidebarCollapsed?'Expandir menú':'Contraer menú'} title={sidebarCollapsed?'Expandir menú':'Contraer menú'}>{sidebarCollapsed?<SidebarExpand width={20} height={20}/>:<SidebarCollapse width={20} height={20}/>}</button></div>
    <div className="sidebar-label">VISTAS</div><nav><button className={page==='board'?'active':''} aria-current={page==='board'?'page':undefined} onClick={()=>setPage('board')}><Kanban width={21} height={21}/><span>Estado</span><b>{pending}</b></button><button className={page==='timeline'?'active':''} aria-current={page==='timeline'?'page':undefined} onClick={()=>{setTimelineAnchorDate(null);setPage('timeline')}}><TableRows width={21} height={21}/><span>Semanal</span><b>{currentWeekTaskCount}</b></button><button className={page==='today'?'active':''} aria-current={page==='today'?'page':undefined} onClick={()=>setPage('today')}><CalendarBlank width={21} height={21}/><span>Diaria</span></button><button className={page==='completed'?'active':''} aria-current={page==='completed'?'page':undefined} onClick={()=>setPage('completed')}><CheckCircle width={21} height={21}/><span>Terminadas</span><b>{tasks.filter(t=>t.status==='completed').length}</b></button><button className={page==='archive'?'active':''} aria-current={page==='archive'?'page':undefined} onClick={()=>setPage('archive')}><Archive width={21} height={21}/><span>Archivo</span><b>{tasks.filter(t=>t.status==='deprecated').length}</b></button><button disabled title="Notas próximamente"><Book width={21} height={21}/><span>Notas</span></button><button disabled title="Notificaciones próximamente"><Bell width={21} height={21}/><span>Notificaciones</span></button></nav>
  ...
  </aside><main>{cloudError&&<div className="cloud-alert" role="alert">{cloudError}</div>}{page==='board'?<Board columns={columns} tasks={tasks} setTasks={setTasks} setColumns={setColumns} openTask={openTask} onViewTimeline={date=>{setTimelineAnchorDate(date);setPage('timeline')}} onViewCompleted={()=>setPage('completed')}/>:page==='timeline'?<Timeline tasks={tasks} columns={columns} setTasks={setTasks} openTask={openTask} holidays={holidays} setHolidays={setHolidays} initialAnchorDate={timelineAnchorDate}/>:page==='today'?<Today tasks={tasks} setTasks={setTasks} openTask={openTask} doneColumnId={doneColumnId}/>:page==='completed'?<CompletedTasks tasks={tasks} setTasks={setTasks} openTask={openTask}/>:page==='archive'?<ArchiveView tasks={tasks} columns={columns} setTasks={setTasks} openTask={openTask}/>:page==='settings'?<AccessSettings/>:<Reports tasks={tasks} openTask={openTask} canShare={Boolean(access?.canManageAccess)}/>}</main>

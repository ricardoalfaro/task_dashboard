...
function Timeline({ tasks, setTasks, openTask }) {
  ...
  const dropTask=event=>{event.preventDefault();const taskId=event.dataTransfer.getData('text/timeline-task');if(taskId)moveTaskToDay(taskId,dayFromPointer(event,event.currentTarget));setDragTargetDay(null)};
  return <div className="timeline-view"><div className="timeline-header"><div><p className="eyebrow">PLANIFICACIÓN SEMANAL</p><h1>Semana</h1><p className="subtitle">Distribuye el trabajo y revisa la carga de cada día.</p></div><button className="primary" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>
    <section className="timeline-panel"><header className="timeline-toolbar"><div className="timeline-navigation"><button className="icon-button" onClick={()=>moveWeek(-1)} aria-label="Semana anterior"><NavArrowLeft width={20} height={20}/></button><button className="timeline-today" onClick={()=>setAnchorDate(today)}>Esta semana</button><button className="icon-button" onClick={()=>moveWeek(1)} aria-label="Semana siguiente"><NavArrowRight width={20} height={20}/></button></div><strong>{formatPeriodDate(range.start)} — {formatPeriodDate(range.end)}</strong><label className="timeline-date-picker"><span>Ir a fecha</span><input type="date" value={anchorDate} onChange={event=>setAnchorDate(event.target.value)} aria-label="Seleccionar una fecha para mostrar su semana"/></label></header>
      <div className="timeline-scroll"><div className="timeline-calendar"><div className="timeline-days">{days.map(day=>{const date=parseISODate(day);const dayCount=visibleTasks.filter(task=>(task.start||task.due)<=day&&task.due>=day).length;return <div className={day===today?'is-today':''} key={day}><span>{date.toLocaleDateString('es-CL',{weekday:'long'}).replace('.','')}</span><b>{date.getDate()}</b><small>{dayCount} {dayCount===1?'tarea':'tareas'}</small></div>})}</div>
        <div className="timeline-body" onDragOver={event=>{event.preventDefault();setDragTargetDay(dayFromPointer(event,event.currentTarget))}} onDragLeave={event=>{if(!event.currentTarget.contains(event.relatedTarget))setDragTargetDay(null)}} onDrop={dropTask}>{dragTargetDay&&<div className="timeline-drop-highlight" style={{left:`calc(${differenceInDays(range.start,dragTargetDay)} * 100% / 7)`}}/>}{visibleTasks.length?visibleTasks.map(task=>{const taskStart=task.start||task.due;const clippedStart=taskStart>range.start?taskStart:range.start;const clippedEnd=task.due<range.end?task.due:range.end;const startColumn=differenceInDays(range.start,clippedStart)+1;const endColumn=differenceInDays(range.start,clippedEnd)+2;return <div className="timeline-task-row" key={task.id}><div draggable onDragStart={event=>{event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/timeline-task',task.id)}} onDragEnd={()=>setDragTargetDay(null)} className={`timeline-task ${task.status==='completed'?'completed':''} ${taskStart<range.start?'continues-before':''} ${task.due>range.end?'continues-after':''}`} style={{gridColumn:`${startColumn} / ${endColumn}`}} title={`Arrastra para mover · ${formatPeriodDate(taskStart)} — ${formatPeriodDate(task.due)}`}><button className="timeline-shift" onClick={()=>moveTaskByDays(task.id,-1)} aria-label={`Mover ${task.title} un día atrás`}><NavArrowLeft width={16} height={16}/></button><button className="timeline-task-open" onClick={()=>openTask(task)}><span className="timeline-task-copy"><strong>{task.title}</strong><small>{task.description||'Sin descripción'}</small><em><CalendarBlank width={13} height={13}/>{fmt(taskStart)} — {fmt(task.due)}</em></span><b>E{task.effort||3}</b></button><button className="timeline-shift" onClick={()=>moveTaskByDays(task.id,1)} aria-label={`Mover ${task.title} un día adelante`}><NavArrowRight width={16} height={16}/></button></div></div>}):<div className="timeline-empty"><CalendarBlank width={30} height={30}/><b>Sin tareas esta semana</b><span>Navega a otra semana o crea una nueva tarea.</span></div>}</div>
      </div></div>
    </section>
  </div>;
}

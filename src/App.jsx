...
function AccessSettings() {
  ...
  useEffect(()=>{load()},[]);
  const save=async event=>{event.preventDefault();setState(current=>({...current,error:''}));try{await authRepository.setMemberRole(supabaseConfig.boardId,form.userId,form.role);setForm({userId:'',role:'viewer'});await load()}catch(error){setState({loading:false,error:error.message})}};
  ...
  return <div className="settings-view"><header><p className="eyebrow">CONFIGURACIÓN</p><h1>Acceso al reporte</h1><p className="subtitle">Administra quién puede consultar el seguimiento externo.</p></header><section className="settings-panel"><div><h2>Agregar acceso</h2><p>El usuario debe existir previamente en Supabase Auth. Usa su ID y asigna el nivel de acceso.</p><form onSubmit={save}><label>ID del usuario<input required value={form.userId} onChange={event=>setForm({...form,userId:event.target.value})} placeholder="UUID de Supabase Auth" pattern="[0-9a-fA-F-]{36}"/></label><label>Rol<select value={form.role} onChange={event=>setForm({...form,role:event.target.value})}><option value="viewer">Solo lectura</option><option value="owner">Propietario</option></select></label><button className="primary">Guardar acceso</button></form></div><div><div className="settings-members-title"><h2>Personas con acceso</h2><span>{members.length}</span></div>{state.error&&<p className="auth-error" role="alert">{state.error}</p>}{state.loading?<p className="settings-empty">Cargando accesos…</p>:members.length?<div className="member-list">{members.map(member=><article key={member.user_id}><div><b>{member.user_id}</b><span>{member.role==='viewer'?'Solo lectura':'Propietario'}</span></div><div><select aria-label={`Rol de ${member.user_id}`} value={member.role} onChange={event=>authRepository.setMemberRole(supabaseConfig.boardId,member.user_id,event.target.value).then(load).catch(error=>setState({loading:false,error:error.message}))}><option value="viewer">Viewer</option><option value="owner">Owner</option></select><button onClick={()=>revoke(member.user_id)} className="member-revoke"><Trash width={16} height={16}/> Revocar</button></div></article>)}</div>:<p className="settings-empty">Aún no hay supervisores agregados.</p>}</div></section></div>;
}
...
function Timeline({ tasks, setTasks, openTask }) {
  ...
  return <div className="timeline-view"><div className="timeline-header"><div><p className="eyebrow">PLANIFICACIÓN SEMANAL</p><h1>Semana</h1><p className="subtitle">Distribuye el trabajo y revisa la carga de cada día.</p></div><button className="primary" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>
    <section className="timeline-panel"><header className="timeline-toolbar"><div><button className="icon-button" onClick={()=>moveWeek(-1)} aria-label="Semana anterior"><NavArrowLeft width={20} height={20}/></button><button className="timeline-today" onClick={()=>setAnchorDate(today)}>Esta semana</button><button className="icon-button" onClick={()=>moveWeek(1)} aria-label="Semana siguiente"><NavArrowRight width={20} height={20}/></button></div><strong>{formatPeriodDate(range.start)} — {formatPeriodDate(range.end)}</strong></header>
      <div className="timeline-scroll"><div className="timeline-calendar"><div className="timeline-days">{days.map(day=><div className={day===today?'is-today':''} key={day} onDragOver={event=>event.preventDefault()} onDrop={event=>{event.preventDefault();const taskId=event.dataTransfer.getData('text/timeline-task');if(taskId)moveTaskToDay(taskId,day)}}><span>{parseISODate(day).toLocaleDateString('es-CL',{weekday:'short'}).replace('.','')}</span><b>{parseISODate(day).getDate()}</b></div>)}</div>
        <div className="timeline-body">{visibleTasks.length?visibleTasks.map(task=>{const taskStart=task.start||task.due;const clippedStart=taskStart>range.start?taskStart:range.start;const clippedEnd=task.due<range.end?task.due:range.end;const startColumn=differenceInDays(range.start,clippedStart)+1;const endColumn=differenceInDays(range.start,clippedEnd)+2;return <div className="timeline-task-row" key={task.id}><div draggable onDragStart={event=>event.dataTransfer.setData('text/timeline-task',task.id)} className={`timeline-task ${task.status==='completed'?'completed':''} ${taskStart<range.start?'continues-before':''} ${task.due>range.end?'continues-after':''}`} style={{gridColumn:`${startColumn} / ${endColumn}`}} title={`${task.title}: ${formatPeriodDate(taskStart)} — ${formatPeriodDate(task.due)}`}><button className="timeline-shift" onClick={()=>moveTaskByDays(task.id,-1)} aria-label={`Mover ${task.title} un día atrás`}><NavArrowLeft width={15} height={15}/></button><button className="timeline-task-open" onClick={()=>openTask(task)}><span>{task.title}</span><b>E{task.effort||3}</b></button><button className="timeline-shift" onClick={()=>moveTaskByDays(task.id,1)} aria-label={`Mover ${task.title} un día adelante`}><NavArrowRight width={15} height={15}/></button></div></div>}):<div className="timeline-empty"><CalendarBlank width={30} height={30}/><b>Sin tareas esta semana</b><span>Navega a otra semana o crea una nueva tarea.</span></div>}</div>
      </div></div>
    </section>
  </div>;
}
...
export function App() {
  ...
  </aside><main>{cloudError&&<div className="cloud-alert" role="alert">{cloudError}</div>}{page==='board'?<Board columns={columns} tasks={tasks} setTasks={setTasks} setColumns={setColumns} openTask={openTask} boardTitle={boardTitle} setBoardTitle={setBoardTitle}/>:page==='timeline'?<Timeline tasks={tasks} setTasks={setTasks} openTask={openTask}/>:page==='today'?<Today tasks={tasks} setTasks={setTasks} openTask={openTask} doneColumnId={doneColumnId}/>:page==='settings'?<AccessSettings/>:<Reports tasks={tasks} openTask={openTask} canShare={Boolean(access?.canManageAccess)}/>}</main>

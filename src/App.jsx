...
const initialColumns = [
  { id: 'todo', title: 'TO-DO', color: themeColors.soft, emoji: '🗂️' },
  { id: 'doing', title: 'DOING', color: themeColors.interaction, emoji: '⚡' },
  { id: 'done', title: 'DONE', color: themeColors.primary, emoji: '✅' },
];
const columnPalette = {todo:themeColors.soft,doing:themeColors.interaction,done:themeColors.primary};
const defaultColumnEmoji = column => {
  const identifier=column?.slug||column?.id;
  return identifier==='todo'?'🗂️':identifier==='doing'?'⚡':identifier==='done'?'✅':'📌';
};
...
function EditableColumnTitle({ value, onChange, autoEdit=false }) {
  ...
  if(editingTitle)return <input className="column-title-input" autoFocus value={draft} onFocus={event=>{if(autoEdit)event.currentTarget.select()}} onChange={event=>setDraft(event.target.value.toUpperCase())} onClick={event=>event.stopPropagation()} onKeyDown={event=>{if(event.key==='Enter')save();if(event.key==='Escape')setEditingTitle(false)}} onBlur={()=>setEditingTitle(false)} aria-label="Nombre de la columna"/>;
...
function Board({ columns, tasks, setTasks, setColumns, openTask, onViewCompleted }) {
  ...
  const [emojiPickerColumn,setEmojiPickerColumn]=useState(null);
  ...
  const setColumnEmoji = (columnId,emoji) => {setColumns(items=>items.map(column=>column.id===columnId?{...column,emoji}:column));setEmojiPickerColumn(null)};
  const createColumn = () => {const id=crypto.randomUUID();setColumns(items=>[...items,{id,slug:`column-${Date.now()}`,title:'NUEVA COLUMNA',color:themeColors.archive,emoji:'📌',isFixed:false}]);setNewColumnId(id);setEmojiPickerColumn(id)};
  ...
  return <div className="board-wrap"><div className="board-header"><div><ProjectSwitcher/><h1>Tablero General</h1><p className="subtitle">Organiza tu trabajo para mantener el foco y los tiempos bajo control.</p></div><button className="primary add-top" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>
    <div className="board" role="list" style={{'--board-column-count':columns.length}}>{columns.map(col=>{const columnTasks=tasks.filter(t=>t.columnId===col.id&&t.status!=='deprecated').sort(boardTaskSort);const list=col.id===doneColumnId?columnTasks.slice(0,12):columnTasks;const isFixed=isFixedColumn(col);return <section className={`column ${isFixed?'fixed-column':'custom-column'}`} key={col.id} draggable={!isFixed} onDragStart={event=>{if(!isFixed)event.dataTransfer.setData('text/column',col.id)}} onDragOver={e=>e.preventDefault()} onDrop={event=>handleColumnDrop(event,col.id)}>
      ...
      <button type="button" className="board-add-column" onClick={createColumn} aria-label="Añadir columna" title="Añadir columna"><Plus width={19} height={19}/></button>
...
function ArchiveView({ tasks, columns, setTasks, openTask }) {
  ...
  const [emptyArchivePrompt,setEmptyArchivePrompt]=useState(false);
  ...
  const emptyArchive=()=>{setTasks(items=>items.filter(item=>item.status!=='deprecated'));setPage(0);setEmptyArchivePrompt(false)};
  return <div className="today-view archive-view archive-table-view"><div className="today-header"><div><ProjectSwitcher/><h1>Archivo <span className="page-title-count">{archivedTasks.length}</span></h1><p className="subtitle">Recupera tareas archivadas o elimínalas definitivamente.</p></div>{archivedTasks.length>0&&<button className="empty-archive-button" onClick={()=>setEmptyArchivePrompt(true)}><Trash width={17} height={17}/> Vaciar la lista de tareas</button>}</div><section className="today-panel"><div className="archive-history-header" aria-hidden="true"><span>Nombre de la tarea</span><span>Esfuerzo</span><span>Fecha de creación</span><span></span><span></span></div><div className="today-list">{archivedTasks.length?visibleTasks.map(task=>{const createdDate=String(task.createdAt||task.due).slice(0,10);const effort=Number(task.effort)||3;return <article className="today-row archive-task-row" key={task.id} onClick={()=>openTask(task)}><div className="today-copy"><h3>{task.title}</h3><p>{task.description||'Sin detalles adicionales.'}</p></div><span className="timeline-effort archive-task-effort" title={`Esfuerzo ${effort} de 5`} aria-label={`Esfuerzo ${effort} de 5`}>{[1,2,3,4,5].map(level=><i className={level<=effort?'is-filled':''} key={level}/>)}</span><span className="today-date archive-created-date"><CalendarBlank width={15} height={15}/> {formatPeriodDate(createdDate)}</span><button className="restore-task" onClick={event=>{event.stopPropagation();restore(task)}}>Restaurar</button><button className="delete-archive-task" onClick={event=>{event.stopPropagation();deletePermanently(task)}} aria-label={`Eliminar ${task.title}`} title="Eliminar definitivamente"><Trash width={17} height={17}/></button></article>}) : <div className="today-empty"><Archive width={32} height={32}/><h3>No hay tareas archivadas.</h3></div>}</div>{archivedTasks.length>perPage&&<footer className="completed-pagination"><span>Página {currentPage+1} de {totalPages}</span><div><button className="icon-button" disabled={currentPage===0} onClick={()=>setPage(current=>Math.max(0,current-1))} aria-label="Página anterior"><NavArrowLeft width={20} height={20}/></button><button className="icon-button" disabled={currentPage===totalPages-1} onClick={()=>setPage(current=>Math.min(totalPages-1,current+1))} aria-label="Página siguiente"><NavArrowRight width={20} height={20}/></button></div></footer>}</section>{emptyArchivePrompt&&<Modal title="Vaciar la lista de tareas" onClose={()=>setEmptyArchivePrompt(false)}><p className="move-prompt-copy">Eliminarás definitivamente las {archivedTasks.length} tareas archivadas. Esta acción no se puede deshacer.</p><footer className="modal-actions"><button type="button" className="secondary" onClick={()=>setEmptyArchivePrompt(false)}>Cancelar</button><button className="primary" onClick={emptyArchive}>Vaciar la lista</button></footer></Modal>}</div>;
}

function Timeline({ tasks, columns, setTasks, openTask, holidays, setHolidays, initialAnchorDate, readOnly=false }) {
  ...
  const dropTask=event=>{if(readOnly)return;event.preventDefault();const day=dayFromPointer(event,event.currentTarget);if(isHoliday(day)){setDragTargetDay(null);return}const resizeTaskId=event.dataTransfer.getData('text/timeline-resize');const taskId=event.dataTransfer.getData('text/timeline-task');if(resizeTaskId)resizeTaskToDay(resizeTaskId,day);else if(taskId)moveTaskToDay(taskId,day);setDragTargetDay(null)};
  const createTaskOnEmptyDay=event=>{if(readOnly)return;const day=dayFromPointer(event,event.currentTarget);if(event.target.closest('.timeline-task')||isHoliday(day))return;openTask(null,undefined,day)};
  ...
  return <div className={`timeline-view ${readOnly?'timeline-readonly':''}`}>{!readOnly&&<div className="timeline-header"><div><ProjectSwitcher/><div className="timeline-title"><h1>Planificador semanal</h1><span>{visibleTasks.length} {visibleTasks.length===1?'tarea':'tareas'}</span></div><p className="subtitle">Distribuye el trabajo y revisa la carga de cada día.</p></div><button className="primary" onClick={()=>openTask(null)}><Plus width={18} height={18} strokeWidth={2.2}/> Nueva tarea</button></div>}
    <section className="timeline-panel"><header className="timeline-toolbar"><button className="timeline-today" onClick={()=>setAnchorDate(today)}>Esta semana</button><div className="timeline-week-switcher"><button className="icon-button" onClick={()=>moveWeek(-1)} aria-label="Semana anterior"><NavArrowLeft width={20} height={20}/></button><strong>{formatPeriodDate(range.start)} — {formatPeriodDate(workEnd)}</strong><button className="icon-button" onClick={()=>moveWeek(1)} aria-label="Semana siguiente"><NavArrowRight width={20} height={20}/></button></div><label className="timeline-date-picker"><span>Ir a fecha</span><input type="date" value={anchorDate} onChange={event=>setAnchorDate(event.target.value)} aria-label="Seleccionar una fecha para mostrar su semana"/></label></header>
      <div className="timeline-scroll" onWheel={navigateWithHorizontalScroll} title="Desplázate horizontalmente para cambiar de semana"><div className="timeline-calendar"><div className="timeline-days">{days.map(day=>{const date=parseISODate(day);const holiday=isHoliday(day);return <div className={`${day===today?'is-today':''} ${holiday?'is-holiday':''}`} key={day}><span>{date.toLocaleDateString('es-CL',{weekday:'long'}).replace('.','')}</span><b>{date.getDate()}</b>{!readOnly&&<div className="timeline-day-menu-wrap"><button className="icon-button timeline-day-menu-trigger" onClick={()=>setHolidayMenu(current=>current===day?null:day)} aria-label={`Opciones para ${formatPeriodDate(day)}`} aria-expanded={holidayMenu===day}><DotsThree width={18} height={18}/></button>{holidayMenu===day&&<div className="timeline-day-menu"><button onClick={()=>{toggleHoliday(day);setHolidayMenu(null)}}>{holiday?'Desmarcar feriado':'Marcar como feriado'}</button></div>}</div>}</div>})}</div>
        <div className="timeline-body" onDoubleClick={readOnly?undefined:createTaskOnEmptyDay} onDragOver={readOnly?undefined:event=>{event.preventDefault();const day=dayFromPointer(event,event.currentTarget);setDragTargetDay(isHoliday(day)?null:day)}} onDragLeave={readOnly?undefined:event=>{if(!event.currentTarget.contains(event.relatedTarget))setDragTargetDay(null)}} onDrop={readOnly?undefined:dropTask}>{days.filter(isHoliday).map(day=><div className="timeline-holiday-overlay" key={day} style={{left:`calc(${differenceInDays(range.start,day)} * 100% / 5)`}}/>)}{dragTargetDay&&<div className="timeline-drop-highlight" style={{left:`calc(${differenceInDays(range.start,dragTargetDay)} * 100% / 5)`}}/>}{timelineLanes.length?timelineLanes.map((lane,laneIndex)=><div className={`timeline-task-row ${verticalDropLane===laneIndex?'is-vertical-drop-target':''}`} key={laneIndex} onDragOver={readOnly?undefined:event=>{if(event.dataTransfer.types.includes('text/timeline-task')){event.preventDefault();event.stopPropagation();const day=dayFromPointer(event,event.currentTarget);setVerticalDropLane(laneIndex);setDragTargetDay(isHoliday(day)?null:day)}}} onDragLeave={readOnly?undefined:event=>{if(!event.currentTarget.contains(event.relatedTarget)){setVerticalDropLane(null);setDragTargetDay(null)}}} onDrop={readOnly?undefined:event=>{const taskId=event.dataTransfer.getData('text/timeline-task');const day=dayFromPointer(event,event.currentTarget);if(taskId){event.preventDefault();event.stopPropagation();if(!isHoliday(day)){moveTaskToDay(taskId,day);reorderTimelineTask(taskId,laneIndex)}}setVerticalDropLane(null);setDragTargetDay(null)}}>{lane.tasks.map(task=>{const taskStart=task.start||task.due;const clippedStart=taskStart>range.start?taskStart:range.start;const clippedEnd=task.due<workEnd?task.due:workEnd;const startColumn=differenceInDays(range.start,clippedStart)+1;const endColumn=differenceInDays(range.start,clippedEnd)+2;const effort=Number(task.effort)||3;return <div key={task.id} draggable={!readOnly} onDragStart={readOnly?undefined:event=>{event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/timeline-task',task.id)}} onDragEnd={readOnly?undefined:()=>{setDragTargetDay(null);setVerticalDropLane(null)}} className={`timeline-task ${task.status==='completed'?'completed':''} ${isOverdue(task)?'overdue':''} ${isOverdueDoing(task)?'overdue-doing':''} ${taskStart<range.start?'continues-before':''} ${task.due>workEnd?'continues-after':''}`} style={{gridColumn:`${startColumn} / ${endColumn}`}} title={task.title}><button className="timeline-task-open" onClick={()=>openTask(task)}><span className="timeline-task-copy"><strong>{task.title}</strong><em><CalendarBlank width={12} height={12}/>{fmt(taskStart)} — {fmt(task.due)}</em></span><span className="timeline-task-meta"><b className={`timeline-status status-${taskStatusTone(task)}`}>{taskStatusLabel(task)}</b><span className="timeline-effort" aria-label={`Esfuerzo ${effort} de 5`} title={`Esfuerzo ${effort} de 5`}>{[1,2,3,4,5].map(level=><i className={level<=effort?'is-filled':''} key={level}/>)}</span></span></button>{!readOnly&&<button type="button" draggable className="timeline-resize-handle" aria-label={`Cambiar duración de ${task.title}`} title="Arrastra para cambiar la duración por días" onClick={event=>event.stopPropagation()} onDragStart={event=>{event.stopPropagation();event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/timeline-resize',task.id)}} onDragEnd={()=>setDragTargetDay(null)}/>}</div>})}</div>):<div className="timeline-empty"><CalendarBlank width={30} height={30}/><b>Sin tareas esta semana</b><span>{readOnly?'Navega a otra semana para revisar sus tareas.':'Navega a otra semana o crea una nueva tarea.'}</span></div>}</div>
      </div></div>
    </section>
  </div>;
}

function Reports({ tasks, openTask, canShare=false, hideShare=false, hideTabs=false }) {
  ...
  const choosePeriod=value=>{const [type,selection]=value.split(':');const year=anchor.getFullYear();setPeriodType(type);if(type==='month')setAnchorDate(`${year}-${String(selection).padStart(2,'0')}-01`);if(type==='quarter')setAnchorDate(`${year}-${String((Number(selection)-1)*3+1).padStart(2,'0')}-01`)};
  ...
  return <div className={`reports ${hideTabs?'reports-without-tabs':''}`}>{(!hideTabs||!hideShare)&&<div className="reports-header">{!hideTabs&&<div className="report-tabs" role="tablist" aria-label="Secciones del reporte"><button type="button" role="tab" aria-selected="true">Resumen General</button></div>}<div className="reports-controls">{!hideShare&&<button className="share-report" disabled={!canShare} onClick={shareReport} title={canShare?'Copiar URL privada del reporte':'Disponible al conectar el acceso externo'}><ShareAndroid width={18} height={18}/> {shareState||'Compartir'}</button>}</div></div>}
    <section className="report-panel report-overview"><div className="report-overview-heading"><div><p className="eyebrow">DISTRIBUCIÓN</p><h2>Estado de las tareas</h2><p>Actividad del {periodLabel}</p></div><div className="report-period-selector"><select aria-label="Año del reporte" value={anchor.getFullYear()} onChange={event=>setPeriodYear(event.target.value)}>{years.map(year=><option key={year}>{year}</option>)}</select><div className={`period-choice-group ${periodType==='quarter'?'is-muted':''}`} aria-label="Meses">{months.map((month,index)=><button type="button" className={periodType==='month'&&anchor.getMonth()===index?'is-selected':''} onClick={()=>choosePeriod(`month:${index+1}`)} key={month}>{month.slice(0,3)}</button>)}</div><div className={`period-choice-group ${periodType==='month'?'is-muted':''}`} aria-label="Trimestres">{[1,2,3,4].map(value=><button type="button" className={periodType==='quarter'&&quarter===value?'is-selected':''} onClick={()=>choosePeriod(`quarter:${value}`)} key={value}>Q{value}</button>)}</div></div></div><div className="report-overview-body"><div className="chart-area"><div className="donut" style={{background:`conic-gradient(${themeColors.primary} 0 ${completed/total*100}%, ${themeColors.interaction} ${completed/total*100}% 100%)`}}><div><strong>{completed+pending}</strong><span>Tareas</span></div></div><div className="legend">{data.map(item=><div key={item.label}><span style={{background:item.color}}></span><p>{item.label}<b>{item.value}</b></p></div>)}</div></div><section className="effort-panel report-effort"><div className="effort-summary"><div className="metric-icon"><Lightning width={22} height={22}/></div><div><p className="eyebrow">NIVEL DE ESFUERZO</p><h2>{effortAverage?effortAverage.toFixed(1):'—'}<span> / 5 promedio</span></h2><p>{periodTasks.length} {periodTasks.length===1?'tarea considerada':'tareas consideradas'} en el período</p></div></div><div className="effort-mini-chart" aria-label="Distribución del nivel de esfuerzo">{effortCounts.map(item=><div key={item.level}><b>{item.count}</b><i style={{height:`${item.count?Math.max(item.count/maxEffortCount*100,18):4}%`}}></i><span>{item.level}</span></div>)}</div></section></div></section>
    <section className="report-task-lists">{[{key:'pending',title:'Tareas pendientes',tasks:pendingTasks},{key:'completed',title:'Tareas terminadas',tasks:completedTasks}].map(list=>{const isCompleted=list.key==='completed';const StatusIcon=isCompleted?Check:CalendarBlank;return <article className={`report-task-list is-${list.key}`} key={list.key}><header><div><h2>{list.title}</h2><span>{list.tasks.length} {list.tasks.length===1?'tarea':'tareas'} · {Math.round(list.tasks.length/total*100)}%</span></div></header><div className="report-task-list-scroll">{list.tasks.length?list.tasks.map(task=>{const date=isCompleted?task.completedAt:task.due;const status=isCompleted?'DONE':task.columnId==='doing'?'DOING':'TO-DO';return <button key={task.id} onClick={()=>openTask(task)}><span className="report-task-list-copy"><b>{task.title}</b>{!isCompleted&&<em className={`report-task-status status-${status.toLowerCase().replace('-','')}`}>{status}</em>}</span><span className="report-task-list-date"><StatusIcon width={15} height={15}/> {formatPeriodDate(date.slice(0,10))}</span></button>}):<div className="completed-empty">No hay tareas {isCompleted?'terminadas':'pendientes'} en este período.</div>}</div></article>})}</section>
  </div>;
}
...
function Today({ tasks, setTasks, openTask }) {
  ...
  return <div className="today-view">
    <div className="today-header">
      <div><ProjectSwitcher/><h1>{greeting} Ricardo <span className="page-title-count">{list.length}</span></h1><p className="subtitle">{workloadSummary}</p></div>
...
function CompletedTasks({ tasks, setTasks, openTask }) {
  ...
  return <div className="today-view completed-tasks-view"><div className="today-header"><div><ProjectSwitcher/><h1>Historial de tareas <span className="page-title-count">{list.length}</span></h1><p className="subtitle">Revisa todas las tareas que ya completaste.</p></div></div>
    <section className="today-panel"><div className="completed-history-header" aria-hidden="true"><span>Nombre de la tarea</span><span>Esfuerzo</span><span>Fecha de término</span><span></span></div><div className="today-list">{list.length?visibleTasks.map(task=>{const completedDate=String(task.completedAt||task.due).slice(0,10);const effort=Number(task.effort)||3;return <article className="today-row completed-task-row" key={task.id} onClick={()=>openTask(task)}><div className="today-copy"><h3>{task.title}</h3><p>{task.description||'Sin detalles adicionales.'}</p></div><span className="timeline-effort completed-task-effort" title={`Esfuerzo ${effort} de 5`} aria-label={`Esfuerzo ${effort} de 5`}>{[1,2,3,4,5].map(level=><i className={level<=effort?'is-filled':''} key={level}/>)}</span><span className="today-date completed-task-date"><CalendarBlank width={15} height={15}/> {formatPeriodDate(completedDate)}</span><button className="icon-button completed-task-archive" onClick={event=>{event.stopPropagation();setArchivingTask(task)}} aria-label={`Archivar ${task.title}`} title="Archivar tarea"><Archive width={19} height={19}/></button></article>}) : <div className="today-empty"><Check width={32} height={32}/><h3>No hay tareas finalizadas.</h3></div>}</div>{list.length>perPage&&<footer className="completed-pagination"><span>Página {currentPage+1} de {totalPages}</span><div><button className="icon-button" disabled={currentPage===0} onClick={()=>setPage(current=>Math.max(0,current-1))} aria-label="Página anterior"><NavArrowLeft width={20} height={20}/></button><button className="icon-button" disabled={currentPage===totalPages-1} onClick={()=>setPage(current=>Math.min(totalPages-1,current+1))} aria-label="Página siguiente"><NavArrowRight width={20} height={20}/></button></div></footer>}</section>{archivingTask&&<Modal title="Archivar tarea" onClose={()=>setArchivingTask(null)}><p className="move-prompt-copy">¿Quieres archivar “{archivingTask.title}”? Podrás restaurarla desde Archivo.</p><footer className="modal-actions"><button type="button" className="secondary" onClick={()=>setArchivingTask(null)}>Cancelar</button><button className="primary" onClick={archiveTask}>Archivar</button></footer></Modal>}</div>;
}
...
export function App() {
  ...
  const [columns,setColumns]=useState(()=>{const saved=JSON.parse(localStorage.getItem('td-columns')||'null')||initialColumns;const legacyColors=['#7c6ee6','#65dcd5','#f5cbcb','#ffe2e2','#c5b3d3','#00a8e8','#007ea7','#003459','#ffef00','#ff7538','#11ee11','#ff66cc'];return saved.map(column=>({...column,color:columnPalette[column.id]||(legacyColors.includes(column.color)?themeColors.archive:column.color),emoji:column.emoji||defaultColumnEmoji(column)}))});
  ...
  if(access?.role==='viewer'||isLocalReportPreview)return <div className="viewer-shell"><header><div><p className="eyebrow">Proyecto</p><strong>{projectName}</strong></div><div className="viewer-header-actions"><button className="secondary viewer-icon-action" disabled title="Descargar PDF próximamente" aria-label="Descargar reporte en PDF próximamente"><Download width={18} height={18}/></button><button className="secondary viewer-icon-action" onClick={cycleTheme} title={`${themeLabel}. Cambiar apariencia`} aria-label={`${themeLabel}. Cambiar apariencia`}><ThemeIcon width={18} height={18}/></button>{!isLocalReportPreview&&<button className="secondary" onClick={()=>authRepository.signOut()}>Cerrar sesión</button>}</div></header>{cloudError&&<div className="cloud-alert" role="alert">{cloudError}</div>}<main><nav className="viewer-tabs" aria-label="Vistas disponibles"><button className={page==='timeline'?'active':''} onClick={()=>{setTimelineAnchorDate(null);setPage('timeline')}}><CalendarBlank width={18} height={18}/> Planificación semanal</button><button className={page==='reports'?'active':''} onClick={()=>setPage('reports')}><ChartBar width={18} height={18}/> Reporte</button></nav><section className="viewer-workspace">{page==='timeline'?<Timeline tasks={tasks} columns={columns} setTasks={setTasks} openTask={task=>{setEditing(task);setModal(true)}} holidays={holidays} setHolidays={setHolidays} initialAnchorDate={timelineAnchorDate} readOnly/>:<Reports tasks={tasks} openTask={task=>{setEditing(task);setModal(true)}} hideShare hideTabs/>}</section></main>{modal&&editing&&<Modal title="Detalle de tarea" onClose={()=>setModal(false)}><ReadOnlyTask task={editing} columnTitle={columns.find(column=>column.id===editing.columnId)?.title}/></Modal>}</div>;
  return <div className={`app-shell ${sidebarCollapsed?'sidebar-collapsed':''}`}><aside><div className="account"><label className="sidebar-search"><MagnifyingGlass width={18} height={18}/><input type="search" placeholder="Buscar" aria-label="Buscar"/></label><button className="account-sidebar-toggle" onClick={()=>setSidebarCollapsed(value=>!value)} aria-label={sidebarCollapsed?'Expandir menú':'Contraer menú'} title={sidebarCollapsed?'Expandir menú':'Contraer menú'}>{sidebarCollapsed?<SidebarExpand width={20} height={20}/>:<SidebarCollapse width={20} height={20}/>}</button></div>
    <div className="sidebar-label">VISTAS</div><nav><button className={`today-nav ${page==='today'?'active':''}`} aria-current={page==='today'?'page':undefined} onClick={()=>setPage('today')}><HomeSimple width={21} height={21}/><span>Bienvenida</span><b className="today-nav-count">{sidebarCollapsed?todayPendingCount:todayTaskCount}</b></button><button className={page==='board'?'active':''} aria-current={page==='board'?'page':undefined} onClick={()=>setPage('board')}><Kanban width={21} height={21}/><span>Tablero General</span><b>{pending}</b></button><button className={page==='timeline'?'active':''} aria-current={page==='timeline'?'page':undefined} onClick={()=>{setTimelineAnchorDate(null);setPage('timeline')}}><CalendarBlank width={21} height={21}/><span>Planificador semanal</span><b>{currentWeekTaskCount}</b></button><button className={page==='completed'?'active':''} aria-current={page==='completed'?'page':undefined} onClick={()=>setPage('completed')}><ClockRotateRight width={21} height={21}/><span>Historial de tareas</span><b>{tasks.filter(t=>t.status==='completed').length}</b></button><button className={page==='archive'?'active':''} aria-current={page==='archive'?'page':undefined} onClick={()=>setPage('archive')}><Archive width={21} height={21}/><span>Archivo</span><b>{tasks.filter(t=>t.status==='deprecated').length}</b></button><button disabled title="Notas próximamente"><Book width={21} height={21}/><span>Notas</span></button><button disabled title="Notificaciones próximamente"><Bell width={21} height={21}/><span>Notificaciones</span></button></nav>

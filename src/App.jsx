...
function ReadOnlyTask({ task, columnTitle }) {
  ...
  return <div className="readonly-task"><span className={`today-status ${task.status}`}>{task.status==='completed'?'Terminada':task.status==='deprecated'?'Archivada':'Activa'}</span><h3>{task.title}</h3>{checklist.length?<section className="readonly-checklist"><h4>Checklist</h4><ul>{checklist.map(item=><li className={item.checked?'checked':''} key={item.id}><span aria-hidden="true">{item.checked?<Check width={14} height={14}/>:null}</span><p>{item.text}</p></li>)}</ul></section>:<p>Sin checklist.</p>}{task.notes&&<p>{task.notes}</p>}{task.link&&<p><a href={task.link} target="_blank" rel="noreferrer">Abrir enlace relacionado</a></p>}<dl><div><dt>Inicio</dt><dd>{formatPeriodDate(task.start||task.due)}</dd></div><div><dt>Fin</dt><dd>{formatPeriodDate(task.due)}</dd></div><div><dt>Esfuerzo</dt><dd>{task.effort||3} de 5</dd></div><div><dt>Estado</dt><dd>{columnTitle||'Sin estado'}</dd></div></dl></div>;
}

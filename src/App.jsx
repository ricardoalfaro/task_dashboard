...
const initialColumns = [
  { id: 'todo', title: 'TO-DO', color: '#ffef00' },
  { id: 'doing', title: 'DOING', color: '#ff7538' },
  { id: 'done', title: 'DONE', color: '#11ee11' },
];
const columnPalette = { todo:'#ffef00', doing:'#ff7538', done:'#11ee11' };
...
function Reports({ tasks, openTask, canShare=false }) {
  ...
  const data=[{label:'Terminadas',value:completed,color:'#11ee11',icon:Check},{label:'Archivadas',value:deprecated,color:'#836de8',icon:Archive},{label:'Pendientes',value:pending,color:'#ffef00',icon:ClipboardText}];
...
export function App() {
  ...
  const [columns,setColumns]=useState(()=>{const saved=JSON.parse(localStorage.getItem('td-columns')||'null')||initialColumns;const legacyColors=['#7c6ee6','#65dcd5','#f5cbcb','#ffe2e2','#c5b3d3','#00a8e8','#007ea7','#003459'];return saved.map(column=>({...column,color:columnPalette[column.id]||(legacyColors.includes(column.color)?'#ff66cc':column.color)}))});

...
- Home greets the owner with `Buen día Ricardo!`; its subtitle dynamically reports the total workload and active overdue count (for example, `Tu carga para hoy es de 3 tareas y tienes 1 atrasada.`). Do not show a separate overdue alert in Home.
...
- On screens at least 1400px wide, Kanban columns widen to 350px for more readable task cards; smaller screens retain the compact width and horizontal scroll behavior.
- In dark mode, the sidebar and expanded project header use borderless, shadowless black translucent glass with a subtle internal gradient and blur so the page canvas glow remains visible behind them. The sidebar gradient starts at 0.7 opacity and fades to 0.25 at its lower edge.
- In dark mode, the Buen día! day-workload panel follows the same borderless, shadowless black-glass treatment; its rows remain transparent with subtle separators and hover feedback.
- In dark mode, overdue rows in Buen día! receive a soft red halo that begins at the upper-right corner and fades toward the left.
- In dark mode, Tablero columns/cards and Planificación surfaces/tasks use the same black translucent gradient glass as the sidebar; state cues remain visible through subtle semantic borders.
- In dark mode, Kanban cards use the same semantic glass fills and borders as Planificación: DONE is translucent green with `rgba(117,214,160,.72)`, active non-overdue work is translucent black with `rgba(14,28,53,.9)`, and overdue work is translucent red with `rgba(255,113,128,.72)`.
- Kanban column headers use a persistent, user-selectable emoji in place of the former colored dot. Clicking it opens an emoji palette; the selected emoji is stored with the column locally and in Supabase.
- Tablero distributes its columns across the available width, reserving only space for a minimal circular `+` column action. Creating a column immediately inserts an empty fourth (or later) Kanban column with a default emoji, its name focused for inline editing, and its emoji palette open; all columns reflow to share the available width before horizontal scrolling is needed.
- In dark mode, Planificación uses a lighter-opacity version of that glass throughout: its container, calendar, controls, day cells, body, and cards remain translucent rather than solid. Its cards have a clearly visible green border and tint for DONE, red for overdue, and a subtle blue-black neutral border otherwise.
- In dark mode, Planificación differentiates workdays with subtle alternating dark zebra veils, dashed separators in the day header, and faint vertical row guides that remain visible beneath dense task cards.
- Theme palette values live in `src/theme-tokens.css`; `src/dark-mode.css` contains dark-only component surfaces. Keep shared layout and component rules in `src/styles.css`, and do not add palette trials or dark selectors there.

...
- The board view has the fixed heading `Tablero de estado`; a shared header across the workspace area to the right of the sidebar contains the project dropdown, currently with the single `Ricardo Alfaro` option. Its heading is not editable.
...
- Task descriptions are persistent checklists. A task with at least one checked item moves to DOING; if all items are unchecked it returns to TO-DO; all items must be checked before it can move to DONE. Kanban drag and drop enforces this: TO-DO cannot go directly to DONE; entering DOING requires a checked item; moving incomplete DOING work to DONE offers to complete all pending items; moving DOING to TO-DO offers to uncheck completed items; DONE cards are not draggable back to active states. Adding an unchecked checklist item to a DONE task returns it to DOING. Notes use a taller field to encourage richer context.
...
- Sidebar navigation is grouped as `VISTAS` (Tablero, Semana, Hoy, Terminadas, Archivo, Notas, Notificaciones) and `OPCIONES` (Reportes only), with Configuración anchored near the bottom above the final Cerrar sesión action. Notas and Notificaciones remain disabled until their workflows are implemented. Archivo uses the same Today-like list anatomy as Hoy and Terminadas while retaining restore and permanent-delete controls. The board's DONE column shows only the 12 most recent tasks and offers `Ver más` to open Terminadas, a Today-like full completed-task history ordered by completion date and paginated at 20 tasks per page. Reports enables Compartir only for an authenticated owner and copies the protected report entry URL.
